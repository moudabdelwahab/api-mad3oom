...
هذا المستودع موقع توثيق ثابت (static) فقط — لا يحتوي على أي منطق مصادقة (authentication) أو لوحة تحكم، وهذا قرار مقصود لإبقاء النطاق واضحًا.

لو احتجت لاحقًا ربط هذا الموقع بتسجيل دخول أو لوحة تحكم (تُدار من تطبيق خارجي أو من منصة Mad3oom الرئيسية)، الأنسب أن يتم ذلك كمشروع/صفحة منفصلة تستهلك نظام الألوان الموحّد الموجود في `css/color-system.css` (المتغيرات: `--color-primary`, `--color-surface`, `--color-text`, ... إلخ) للحفاظ على الاتساق البصري، بدل دمج كود مصادقة داخل هذا المستودع. هذا الملف هو الموضع الوحيد المُصمَّم لإعادة الاستخدام بين المشروعات.
